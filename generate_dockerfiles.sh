...
gen_dockerfile() {
  ...
  for PLATFORM in ${PLATFORM_FLAVORS[@]}; do
    for DOCKERTAG in ${DOCKERTAG[@]}; do
       DOCKERFILE_TARGET="${PLATFORM}/${DOCKERTAG}/${JVM_MAJOR}/${JVM_MINOR}b${JVM_BUILD}/${JVM_PACKAGE}/${JAVA_JCE}/Dockerfile"
       ...
       DEFAULT_DOCKER_BASEIMAGE=tomaer/alpine:edge-utc
